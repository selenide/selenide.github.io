...
Today is October, 25.
Today, 12 years ago the first commit in Selenide project was created.  
...
* [Static initialisation deadlock](#fix-deadlock-in-static-initialization)
...
Compare to this, update from Java 11 to Java 17 is much more simple. There is nothing to do there, really. 
...
### Fixed static initialisation deadlock {#fix-deadlock-in-static-initialization}
...
> More details about [static initialisation deadlock](https://blog.palantir.com/using-static-analysis-to-prevent-java-class-initialization-deadlocks-c2f31ca967d6).

To fix the problem, we had to rename base class `Condition` to `WebElementCondition`. 
...
But users sometimes used them. And even complained that these methods didn't work.
...
We removed all methods like `addListener()` with parameter of type `org.openqa.selenium.support.events.WebDriverEventListener`.
