...
* [Новый метод `$.selectRadio()`](#method-selectradio)
* [`$.setValue()` теперь понимает кнопки radio](#setvalue-radio-buttons)
* [Нельзя менять значение в поле readonly](#no-set-value-readonly)
* [`$.setValue()` учитывает атрибут `maxlength`](#setvalue-maxlength)
* [Исправлена проблема с незакрывающимися браузерами FireFox](#fix-unclosed-firefox)
* [Новости](#news)
...
## Метод `$.setValue()` теперь понимает и кнопки radio {#setvalue-radio-buttons}
...
## Метод `$.setValue()` теперь учитывает атрибут `maxlength` {#setvalue-maxlength}
