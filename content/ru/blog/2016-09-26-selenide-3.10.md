...
description: ""
category:
headerText: "Скачивание в жизни не главное"
tags: []
---
Привет!
...
* [Прокси-сервер выключен по умолчанию](#proxy-disabled-by-default)
* [Поддержка множественных селектов](#multiple-selects)
* [Отчёт только для упавших тестов](#report-only-for-failed-tests)
* [Исправлен парсинг заголовка `Content-Disposition` с кодировкой](#content-disposition-encoding)
* [Исправлено проглатывание исключений](#swallowed-exceptions)
* [`open(url)` работает с URL большими буквами](#open-url-uppercase)
* [Вернули старый закомментированный тест](#uncommented-old-test)
* [Новости](#news)
...
## Добавлена поддержка множественных селектов {#multiple-selects}
...
## Можно печатать отчёт только для упавших тестов {#report-only-for-failed-tests}
...
## Исправили парсинг заголовка `Content-Disposition` с указанием кодировки {#content-disposition-encoding}
...
## Исправили багу 401: "Selenide swallows exception in some cases" {#swallowed-exceptions}

Никогда не пишите `catch (Throwable)`. Никогда!

См. [issue 401](https://github.com/selenide/selenide/issues/401)

## Метод `open(url)` теперь работает и для URL большими буквами {#open-url-uppercase}
