---
slug: "selenide-5.15.0"
date: 2020-09-26
title: "Вышла Selenide 5.15.0"
description: ""
category:
headerText: "Обкликайся и обкачайся"
tags: []
---
Всем привет!
...
* [Настройка `Configuration.pageLoadTimeout`](#page-load-timeout)
* [Клик с параметрами](#click-with-options)
* [Скачивание файла с параметрами](#download-with-options)
* [Работа с LocalStorage](#local-storage)
* [Текст элемента без потомков](#own-text)
* [Ускорили большие фильтрованные коллекции](#speed-up-collections)
* [Добавили проверку "href"](#href-checks)
* [Добавили опцию хрома "--no-sandbox"](#no-sandbox)
* [Явная ошибка, если не удалось создать папку для скачивания](#error-if-failed-to-create-download-folder)
* [Обновились на WebDriverManager 4.2.2](#update-webdriver-manager)
* [Видео-обзор релиза](#video-review)
* [Новости](#news)


## Добавили настройку `Configuration.pageLoadTimeout` <br/> (по умолчанию 30 секунд) {#page-load-timeout}
...
## Добавили универсальный метод клика<br> с параметром `ClickOptions` {#click-with-options}
...
Поэтому мы добавили метод, в который можно явно передавать параметр - способ клика. 
...
## Добавили универсальный метод скачивания файла<br> с параметром `DownloadOptions` {#download-with-options}
...
## Добавили методы для работы с LocalStorage {#local-storage}
...
## Добавили проверки для текста элемента без потомков {#own-text}
...
А иногда хочется проверить только текст самого элемента, без потомков. Для этого мы добавили две новых проверки:
...
## Ускорили работу с большими фильтрованными коллекциями {#speed-up-collections}
...
> И ещё мощный способ ускорить работу с коллекциями (и вообще тесты) - это использование JavaScript.  
> 
> См. [Трюки с JavaScript](https://www.youtube.com/watch?v=be_cTwayRQc&ab_channel=FestGroup&t=29m57s)
...
## Селенид теперь явно кидает ошибку {#error-if-failed-to-create-download-folder}
...
Смотрите [видео-обзор](https://youtu.be/txozPtQIqiE) данного релиза. 
