...
description: ""
category:
headerText: "Обновлённый алгоритм ожиданий. Теперь точно никаких StaleElementException."
tags: []
---
Здрасьте!
...
* [Механизм ожиданий](#waiting-algorithm)
* [Скриншоты](#screenshots)
* [`$.shouldHave(value())` игнорирует невидимые символы](#shouldhave-ignores-invisible-characters)
* [Режим кликов через JavaScript](#clicking-with-javascript)
* [Новый метод `$.doubleClick()`](#method-doubleclick)
* [Чейнинг `$.hover()`, `$.contextClick()`, `$.dragAndDropTo()`](#chain-hover-contextclick-draganddropto)
* [Исправлены Sizzle селекторы без jQuery](#fix-sizzle-selectors)
* [Новый метод `WebDriverRunner.hasWebDriverStarted()`](#haswebdriverstarted)
* [`$.setValue()` в режиме fastSetValue генерирует событие `input`](#setvalue-triggers-input-event)
* [Обновили commons-codec до 1.10](#commons-codec-1.10)
* [Статистика скачиваний](#download-statistics)
...
## Метод `$.shouldHave(value("john"))` игнорирует невидимые символы (пробелы, табуляции, переводы строк). {#shouldhave-ignores-invisible-characters}
...
## Добавили метод `$.doubleClick()` {#method-doubleclick}
...
## Методы `$.hover()`, `$.contextClick()`, `$.dragAndDropTo()` теперь можно _чейнить_ {#chain-hover-contextclick-draganddropto}
...
## Исправлена работа Sizzle селекторов на страничках без jQuery {#fix-sizzle-selectors}

> Спасибо @Gert за этот pull request!

## Добавлен метод `WebDriverRunner.hasWebDriverStarted()` {#haswebdriverstarted}
...
## Метод `$.setValue()` в режиме `fastSetValue` теперь генерирует событие `input` {#setvalue-triggers-input-event}
...
## Теперь Selenide зависит от более последней версии commons-codec 1.10 {#commons-codec-1.10}
...
## Разрешите похвастаться {#download-statistics}
...
<center>
  <img src="/images/2015/05/selenide_downloads_unique_ips.png" width="800"/>
</center>
<br>

И количество непосредственно скачиваний:
<center>
  <img src="/images/2015/05/selenide_downloads.png" width="800"/>
</center>
