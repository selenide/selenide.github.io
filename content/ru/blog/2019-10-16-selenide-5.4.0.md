...
* [Исправили ошибку `IllegalStateException`](#fix-illegal-state-exception)
* [Метод `close()` теперь всегда закрывает браузер](#close-always-closes-browser)
* [Короткие сообщения об ошибках](#restore-short-error-messages)
* [Метод `using` для переключения вебдрайверами](#add-method-using)
* [Защита от ошибки с xpath](#add-xpath-sanity-check)
