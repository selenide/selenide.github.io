...
description: ""
category:
headerText: "Проснитесь, у нас снова релиз!"
tags: []
---
Добрый вечер!  
...
* [Закрыть алерт перед скачиванием файла](#close-alert-before-download)
* [Доработали проверку `Condition.textCaseSensitive`](#improve-text-case-sensitive)
* [Селекторы `byTextCaseInsensitive` и `withTextCaseInsensitive`](#case-insensitive-selectors)
* [Метод `Driver.getSessionId()`](#get-session-id)
* [Уменьшили таймауты вебдрайвера](#shorten-webdriver-timeouts)
...
# Теперь можно закрыть алерт перед скачиванием файла {#close-alert-before-download}
...
# Добавили селекторы `byTextCaseInsensitive` и `withTextCaseInsensitive` {#case-insensitive-selectors}
...
Поэтому пришлось наколбасить [хитрый JS код](https://github.com/selenide/selenide/blob/master/src/main/resources/find-elements-by-text.js) для обхода всего дерева элементов.
...
# Добавили метод `Driver.getSessionId()` {#get-session-id}
...
# Уменьшили селенимовские таймауты {#shorten-webdriver-timeouts}
