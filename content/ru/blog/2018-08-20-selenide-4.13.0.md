---
slug: "selenide-4.13.0"
date: 2018-08-20
title: "Вышла Selenide 4.13.0"
description: ""
category:
headerText: "Под звуки салюта"
tags: []
---
Всем привет!
...
* [Метод `$.lastChild()`](#method-lastchild)
* [Проверки для коллекций с кастомным таймаутом](#collection-checks-with-timeout)
* [Метод `Selenide.download(url)`](#method-download)
* [Обновились до Selenium 3.14.0](#upgrade-selenium)
* [`switchTo().alert()` кидает `NoAlertPresentException`](#switchto-alert-exception)
* [Исправили сообщение об ошибке из `$.selectOptionByValue()`](#fix-selectoptionbyvalue-error)
* [Исправили имя скачиваемого файла](#fix-downloaded-file-name)
* [Обновились до WebdriverManager 2.2.5](#upgrade-webdrivermanager)
* [Обновились до HtmlUnit 2.32.1](#upgrade-htmlunit)
* [Починили тесты для Firefox](#fix-firefox-tests)
* [Новости](#news)

<br>

# Добавили метод `$.lastChild()` {#method-lastchild}
...
# Добавили проверки для коллекций с кастомным таймаутом {#collection-checks-with-timeout}
...
# Добавили метод `Selenide.download(url)` {#method-download}
...
# Метод `switchTo().alert()` теперь кидает `NoAlertPresentException` {#switchto-alert-exception}
...
# Исправили сообщение об ошибке из метода `$.selectOptionByValue()` {#fix-selectoptionbyvalue-error}
...
# Исправили все селенидовские тесты для Firefox {#fix-firefox-tests}
