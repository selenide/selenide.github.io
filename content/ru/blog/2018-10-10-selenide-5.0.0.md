...
* [Два браузера в одном тесте](#two-browsers-in-test)
* [Chrome по умолчанию](#chrome-by-default)
* [Браузер больше не максимизируется по умолчанию](#no-maximize-by-default)
* [Браузер больше не открывается автоматически](#no-auto-open-browser)
* [Подчистили старые настройки](#cleanup-unused-settings)
* [Убрали зависимость от `junit5-api`](#remove-junit5-api-dependency)
* [По мелочам](#minor-changes)
* [Технические изменения](#technical-changes)
* [Новости](#news)
* [Нам 7 лет!](#seven-years-old)
* [Как у вас прошло обновление?](#how-your-upgrade-went)

# Возможность открывать два браузера в одном тесте {#two-browsers-in-test}
...
Обычно такое желание возникает для того, чтобы, скажем, 
1. одним браузером в админке поменять какую-то настройку
2. и другим браузером проверить, что на страничке пользователя эта настройка отобразилась.

Но правильное решение в таком случае - тестировать админку отдельно и пользовательскую страничку отдельно. 
А подготавливать окружение (в т.ч. менять настройки) можно как угодно (rest запрос, прямой запрос в базу и т.п.), но **только не через UI**.

> Вы ведь тестируете UI, а значит, априори *не должны ему доверять*.
...
См. [issue 354](https://github.com/selenide/selenide/issues/354) и [PR 801](https://github.com/selenide/selenide/pull/801)
...
# Теперь Селенид по умолчанию использует Chrome {#chrome-by-default}
...
Этот аргумент давно неактуален, т.к.
1. Firefox теперь тоже требует установки geckodriver, и
2. Селенид умеет устанавливать их автоматически.

Зато Chrome, по нашему опыту, быстрее и надёжнее. Поэтому наступила эпоха Хрома. 
 
См. [issue 811](https://github.com/selenide/selenide/issues/811)  --  спасибо [Aliaksandr Rasolka](https://github.com/rosolko) за [PR 812](https://github.com/selenide/selenide/pull/812)

<br>

# Селенид больше *не* максимизирует браузер по умолчанию {#no-maximize-by-default}
...
См. [issue 810](https://github.com/selenide/selenide/issues/810)  --  спасибо [Aliaksandr Rasolka](https://github.com/rosolko) за [PR 812](https://github.com/selenide/selenide/pull/812)

<br>

# Селенид больше не открывает браузер автоматически {#no-auto-open-browser}

Действия и проверки - например,
* `$("#name").click()` и
* `$$("#list").shouldHave(size(1))`)

теперь кидают ошибку, если браузер не открыт (или уже закрыт).
...
#### 1. Статические поля ПО
Поля пэдж обжекта были объявлены статическими, а браузер закрывался после каждого теста.  
Статические поля инициализировались только один раз. Каждый следующий тест не переинициализировал их, а
получал `SelenideElement` от предыдущего теста с устаревшей ссылкой на вебдрайвер (уже закрытый).

Решение: сделать поля нестатическими. Статики - зло.
    
#### 2. Поля класса в TestNG
Поля тест-класса (в случае TestNG) инициализировались не в `setUp` методе, а прямо в объявлении полей.
...
См. [issue 809](https://github.com/selenide/selenide/issues/809)

<br>

# Подчистили старые настройки и фичи {#cleanup-unused-settings}

См. [issue 806](https://github.com/selenide/selenide/issues/806)  --  спасибо [Aliaksandr Rasolka](https://github.com/rosolko) за [PR 812](https://github.com/selenide/selenide/pull/812)
...
* [817](https://github.com/selenide/selenide/issues/817) fix "FirefoxDriverFactory overwrites Firefox profile provided by Configuration"  -- спасибо [Boris Osipov](https://github.com/BorisOsipov) за [PR 821](https://github.com/selenide/selenide/pull/821)
...
# И технические изменения (которые вас, скорее всего, не затронут) {#technical-changes}
...
<center>
  <img src="/images/2018/10/selenide-first-commits.png" width="800"/>
</center>

<br>

# А как у вас прошло обновление? {#how-your-upgrade-went}
