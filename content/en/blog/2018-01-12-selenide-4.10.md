...
description: ""
category:
headerText: "Fast $$ and headless on the grid"
tags: []
---
...
* [Method `$.scrollIntoView()`](#scroll-into-view)
* [Headless browsers on Selenium Grid](#headless-on-grid)
* [Custom capabilities for Chrome and Firefox](#custom-capabilities)
* [Optimized `$$` iteration](#optimize-collection-iteration)
* [`User-Agent` header for downloads](#user-agent-header)
* [`byText`/`withText` understand unbreakable spaces](#unbreakable-spaces)
* [Upgraded phantomjsdriver](#upgrade-phantomjsdriver)
* [Upgraded htmlunit and guava](#upgrade-htmlunit-guava)
* [News](#news)

<br>

# We added method `$.scrollIntoView()` {#scroll-into-view}
...
# Now tou can run _headless_ browsers on Selenium Grid {#headless-on-grid}
...
# Now you can set custom capabilities for Chrome and Firefox {#custom-capabilities}
...
# We optimized performance of `$$` iteration {#optimize-collection-iteration}
...
# We added "User-Agent" header when downloading a file {#user-agent-header}
...
# Now methods `byText` and `withText` understand unbreakable spaces {#unbreakable-spaces}
...
# Upgraded to phantomjsdriver 1.4.4 {#upgrade-phantomjsdriver}

Which in turn was upgraded to selenium-java 3.8.1 (and doesn't have any other changes).  

<br>

# Upgraded to htmlunit 2.29 and guava:23.6-jre {#upgrade-htmlunit-guava}
