...
description: ""
category:
headerText: "Safari support, multi-threaded TestNG run support and upgrade to Selenium 2.40.0"
tags: []
---
...
* [Safari browser support](#safari-browser-support)
* [Condition `or`](#or-condition)
* [Method `ScreenShooter.to(folder)`](#screenshooter-to-folder)
* [`$.download()` throws exceptions on server errors](#download-throws-exceptions)
* [Chrome window is maximized](#chrome-window-maximized)
* [`shouldHave(text())` ignores non-breaking space](#nbsp-in-text-condition)
* [Closing browser windows in multi-threaded TestNG runs](#close-browser-multithreaded-testng)
* [Including webdriver exception in error message](#include-webdriver-exception)
* [Logging exception when screenshot fails](#log-screenshot-exception)
* [Page source saved in correct encoding (UTF-8)](#page-source-utf8)
* [Removed INFO messages about "reportsUrl" and "BUILD_URL"](#remove-info-messages)
* [Using `jQuery` instead of `$`](#jquery-instead-of-dollar)
* [Upgraded to Selenium 2.40.0, HtmlUnit 2.14, TestNG 6.8.8](#upgrade-selenium-2-40-0)

## New functionality

### Added support for Safari browser {#safari-browser-support}
...
### Added condition `or` {#or-condition}
...
### Added method `ScreenShooter.to(folder)` {#screenshooter-to-folder}
...
### Method `$.download()` throws `FileNotFoundException` (if server returns 40x error) or `RuntimeException` (for 50x status) {#download-throws-exceptions}
...
### Chrome browser window is also maximized. {#chrome-window-maximized}
...
### Method `$.shouldHave(text())` also ignores the \\u00a0 character (so called "non-breaking space") {#nbsp-in-text-condition}
...
### Improved automatic closing of browser window in case of multi-threaded tests run with TestNG {#close-browser-multithreaded-testng}
...
### Including webdriver exception {#include-webdriver-exception}
...
### If Selenide fails to take screenshot, it logs corresponding exception with stack trace. {#log-screenshot-exception}
...
### Selenide saves page source in correct encoding (UTF-8) {#page-source-utf8}
...
### Using "`jQuery`" word instead of "`$`" {#jquery-instead-of-dollar}
...
### Upgraded to Selenium 2.40.0, HtmlUnit 2.14 and TestNG 6.8.8 {#upgrade-selenium-2-40-0}
