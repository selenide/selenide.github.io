...
* [`Commands` return `SelenideElement`](#commands-return-selenideelement)
* [Fixed `$.setValue(null)`](#fix-setvalue-null)
* [Soft asserts in `@BeforeAll`/`@AfterAll` (JUnit 5)](#soft-asserts-in-beforeall-afterall)
* [Fixed javadoc jar](#fix-javadoc-jar)

<br>

# Make `Commands` return `SelenideElement` instead `WebElement` {#commands-return-selenideelement}
...
# Fixed method `$.setValue(null)` {#fix-setvalue-null}
...
# Enabled soft asserts in @BeforeAll and @AfterAll methods (in JUnit 5) {#soft-asserts-in-beforeall-afterall}
...
# Fixed file [selenide-5.17.2-javadoc.jar](https://search.maven.org/remotecontent?filepath=com/codeborne/selenide/5.17.2/selenide-5.17.2-javadoc.jar) 
  now it contains javadoc for all classes.   {#fix-javadoc-jar}
