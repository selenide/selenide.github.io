...
description: ""
category:
headerText: "Disabled proxy, multiple selects, reports tuning"
tags: []
---
...
* [Proxy server disabled by default](#proxy-disabled-by-default)
* [Multiple selects support](#multiple-selects)
* [Print report only for failed tests](#report-only-for-failed-tests)
* [Fixed parsing of `Content-Disposition` header with encoding](#content-disposition-encoding)
* [Fixed swallowed exceptions](#swallowed-exceptions)
* [`open(url)` works with URL in upper case](#open-url-uppercase)
* [Uncommented an old test](#uncommented-old-test)
* [News](#news)

## Proxy server is disabled by default {#proxy-disabled-by-default}
...
## Added support for multiple selects {#multiple-selects}
...
## Now you can print report only for failed tests {#report-only-for-failed-tests}
...
## Fixed bug with parsing http header `Content-Disposition` containing encoding {#content-disposition-encoding}
...
## Fixed bug 401: "Selenide swallows exception in some cases" {#swallowed-exceptions}

Never ever write `catch (Throwable)`. Never!

See. [issue 401](https://github.com/selenide/selenide/issues/401)

## Method `open(url)` now works for URL in upper case {#open-url-uppercase}
...
## Uncommented one old test {#uncommented-old-test}
