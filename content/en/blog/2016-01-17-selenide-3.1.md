...
* [`$().download()` does not allow invisible links](#download-invisible-links)
* [`switchTo(...)` methods can now wait](#switchto-wait)
* [New methods `byName`, `byXpath`, `byLinkText`, `byPartialLinkText`, `byId`](#by-selectors)
* [`exactTextCaseSensitive` checks the entire string](#exact-text-case-sensitive)
* [Deprecated `$(WebElement, selector)` and `$$(WebElement, selector)`](#deprecate-find-with-webelement)
* [New method `$.getValue()`](#get-value)
* [Bundled phantomjsdriver 1.2.1](#phantomjsdriver-included)
* [Upgraded to selenium-java 2.49.0](#upgrade-to-selenium-2.49.0)
...
## Method `$().download()` does not allow invisible links {#download-invisible-links}
...
## Methods `switchTo(...)` now can wait if needed {#switchto-wait}
...
## Added methods `byName`, `byXpath`, `byLinkText`, `byPartialLinkText`, `byId` {#by-selectors}
...
## Condition`$.shouldHave(exactTextCaseSensitive("..."))` checks the entire string {#exact-text-case-sensitive}

Before Selenide 3.1 it checks only a substring. We fixed this bug.

## Methods `$(WebElement, selector)` and `$$(WebElement, selector)` marked as deprecated {#deprecate-find-with-webelement}
...
## Added method `$.getValue()` {#get-value}

Actually this is just an alias for `$.val()`.

## Selenide includes phantomjsdriver 1.2.1 out of the box {#phantomjsdriver-included}

Now you don't need to include additional dependencies to use PhantomJS in your tests.

## Upgraded to selenium-java 2.49.0 {#upgrade-to-selenium-2.49.0}

See [selenium 2.49.0 changelog]({{< selenium-changelog >}}).
