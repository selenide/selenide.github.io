...
So, the FOLDER method hasn't worked well with Selenium Grid up until now for large files (say, over 100 MB).
Also, there was no reliable way to wait for the download to complete.
...
NB! It's important to update Selenium Grid as well.
If it stays outdated, updating only the client side (i.e., your Selenide version) won't help.
